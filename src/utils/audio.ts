// Web Audio API procedural sound synthesizer for Life RPG (zero external assets needed)
...
class SoundFX {
  ...
  // Play satisfying quest complete chime (bell chord)
  playQuestComplete() {
  ...
  // Play celebratory level up triumphant fanfare
  playLevelUp() {
    if (!this.enabled) return;
    try {
      this.initCtx();
      ...
      const fanfare = [
        ...
        { freq: 880.0, time: 0.36, dur: 0.45 }, // A5 triumph
      ];
  ...
  // Play sword slash / combat impact sound
  playSlash() {
    if (!this.enabled) return;
    try {
      this.initCtx();
  ...
  // Heavy boss hit / critical strike
  playBossHit() {
    if (!this.enabled) return;
    try {
      this.initCtx();
